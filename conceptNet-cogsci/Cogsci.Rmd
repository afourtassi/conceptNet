...
```{r message=FALSE}
clusters_evolution <- function(vocab, n_clusters, min_age, max_age, randomization) {
  
  if(randomization == "none") {
    # do nothing
  } else if(randomization == "random_aoa") {
    vocab <- randomize_aoa(vocab) # TODO handle randomization, write "randomize_aoa"" in random_helper.r
  } else if(randomization == "to_nearest_aoa") {
    vocab <- randomize_aoa_to_nearest(vocab)
  } else if(randomization == "within_last_clustering") {
...
measure_cluster_dynamics <- function(language, n_clusters, measures, randomizations, samples) {
    ...
    measurements[["true_vocab_sizes"]] <-
      tibble(age = min(vocab$age):max(vocab$age),
             measure = "true_vocab_size",
             randomization = NA, # same for all randomizations
             sample = NA,
    ...
    for (measure in measures) {
      value <- run_measure(true_clusters_evo, measure)
    ...
    for (randomization in randomizations) {
      ...
      for (sample in 1:samples) {
        ...
        for (measure in measures) {
          ...
          measurements[[paste(randomization, measure, sample, sep = "__")]] <-
            tibble(age = min(vocab$age):max(vocab$age),
                   measure = measure,
                   randomization = randomization,
                   sample = sample,
...
run_experiment <- function(languages, measures, randomizations, cluster_counts, samples) {
  measurements = list()
  ...
  for(language in languages) {
    ...
    for(n_clusters in cluster_counts) {
      ...
      measurements[[length(measurements) + 1]] <- 
        measure_cluster_dynamics(language, n_clusters, measures, randomizations, samples) %>%
...
randomizations <- c("random_aoa", "to_nearest_aoa", "within_last_clustering")
