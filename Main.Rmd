...
full_vocab <- make_vocab_dataframe(lang="English (American)",
  ...
  select(uni_lemma, definition, age)
```
...
To begin our analysis, we only consider the words for which feature data are available, an overlap of `r nrow(vocab)` in our data compared to Hills' 130.

## Word2Vec

## Build Networks

Using vocabulary obtained by 30 months and the same McRae features as Hills, we construct networks from perceptual and conceptual features. We track the number of features that generate each edge.

```{r message=FALSE}
# perceptual and conceptual McRae features
...
perceptual_links <- make_feature_links(vocab = hills_vocab,
                                       feature_types = perceptual_features) %>%
  rename(perceptual = shared)

conceptual_links <- make_feature_links(vocab = hills_vocab,
                                       feature_types = conceptual_features) %>%
  rename(conceptual = shared)

# get word2vec distance between words in vocabulary
w2v_links <- make_w2v_links(vocab = hills_vocab) #TODO still problem w/ vocab matching w2v data

links <- perceptual_links %>%
  full_join(conceptual_links, by=c("from", "to")) %>%
  full_join(w2v_links, by=c("from", "to"))

full_network <- graph_from_data_frame(links, vertices = hills_vocab, directed = FALSE) %>%
  as_tbl_graph()
```
...
plot_network(full_network %E>% filter(w2v_similarity >= 0.5), remove_isolated = TRUE, labels=TRUE, frame=TRUE)
```
